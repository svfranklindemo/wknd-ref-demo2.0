...
if (shouldLoadZDP) {
    const zdpId = getSearchParam('zdp-id') || sessionStorage.getItem('zdp-id') || ''

  // Getting env
  const zdpEnv = getSearchParam('zdp-env') || sessionStorage.getItem('zdp-env') || ''

  // Getting token
  const zdpToken = getSearchParam('zdp-token') || sessionStorage.getItem('zdp-token') || ''

  // Getting zdp preview
  const zdpPreview = getSearchParam('zdp-preview') || sessionStorage.getItem('zdp-preview') || ''
    ...
    (async () => {
        try {
            ...
            const response = await fetch(`${demoPilotDomain}/auth`, {
                ...
                body: JSON.stringify({ "accessToken": zdpToken }),
            });
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Conditionally import uploadAsset based on stage/prod
        let uploadAsset;
        const isProd = getSearchParam('zdpEnv') === 'prod';
        if (isProd) {
            const { uploadAsset: prodUploadAsset } = await import('./asset-upload.js');
            uploadAsset = prodUploadAsset;
            console.log('Using production uploadAsset');
        ...
        const domain = getSearchParam('zdpEnv') === 'prod' 
            ? 'copilot.adobedemo.com' 
            : 'stage.pilot.adobedemo.com';
